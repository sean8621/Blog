...
## 3.line-height设置为1会怎么样
1. line-height是一行文本所占据的垂直空间大小。
2. 当line-height:1时：此时行高和字体的大小相同。举个例子，要是字体大小设定为16px，那么行高也就是16px。
3. 实际效果：
- 文本可能会重叠：字体设计的时候，会在字符的顶部和底部预留一些额外的空间，这就是 “行间距”。当line-height:1时，行高变得过小，可能会让字符的顶部或者底部出现重叠的情况。
- 行与行之间没有间距：正常情况下，相邻两行文本之间会有一定的间距。但当line-height:1时，这个间距就消失了，使得文本看起来十分紧凑。
- 不同字体效果有差异：由于不同字体的内部设计不一样，所以line-height:1在不同字体上产生的效果也不尽相同。比如，有些字体的部分字符可能会超出字体的标准尺寸，在这种情况下就更容易出现文本重叠的现象。